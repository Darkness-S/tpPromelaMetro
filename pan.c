...
{	int j, h = now._nr_pr;
	...
	case 0:	/* Deplacement */
		...
		((P0 *)pptr(h))->pprime = 0;
#ifdef VAR_RANGES
		logval("Deplacement:Dir", ((P0 *)pptr(h))->Dir);
		logval("Deplacement:DirOth", ((P0 *)pptr(h))->DirOth);
		logval("Deplacement:p", ((P0 *)pptr(h))->p);
		logval("Deplacement:pprime", ((P0 *)pptr(h))->pprime);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.R1toR2 = addqueue(calling_pid, 1, 0);
		now.R2toR1 = addqueue(calling_pid, 2, 0);
		now.ReqRi = addqueue(calling_pid, 3, 0);
		R1 = 0;
		R2 = 0;
#ifdef VAR_RANGES
#endif
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.R1toR2) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.R1toR2) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.R1toR2) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.R1toR2) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.R2toR1) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.R2toR1) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.R2toR1) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.R2toR1) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.ReqRi) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.ReqRi) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.ReqRi) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.ReqRi) )))
			    ) return 0; break;
...
{	/* int i; */
	...
	printf("	chan ReqRi (=%d):	len %d:\t", now.ReqRi, q_len(now.ReqRi));
	c_chandump(now.ReqRi);
}
void
...
{	/* int i; */
	...
	case 1:
		...
		break;
	case 0:
		printf("local vars proc %d (Deplacement):\n", pid);
	printf("	mtype  Dir:	%d\n", ((P0 *)pptr(pid))->Dir);
	printf("	short  p:	%d\n", ((P0 *)pptr(pid))->p);
	printf("	mtype  DirOth:	%d\n", ((P0 *)pptr(pid))->DirOth);
	printf("	short  pprime:	%d\n", ((P0 *)pptr(pid))->pprime);
		break;
	}
}
void
...
{
	switch (x) {
	case 1: Printf("Null"); break;
	case 2: Printf("Ouest"); break;
	case 3: Printf("Est"); break;
	default: Printf("%d", x);
	}
}
void
